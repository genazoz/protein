...
`;
const ImageProteinWrapper = styled(ImageWrapper)`
...
`;
const CircleWrapper = styled(ImageWrapper)`
  z-index: 0;
  top: 0;
...
const SocialLink = styled(Link)`
  margin: 0 10px 0 0;
`;
const ProteinBarWrapper = styled.div`
  position: absolute;
  z-index: 1;
  width: 100%;
  height: 100%;
...
function Preview({ destinationIndex }) {
  React.useEffect(() => {
  ...
  const { previewSubmitHovered, setPreviewSubmitEnter } =
  ...
  const parallax = (e) => {
    ...
    if (window.innerWidth < 1100) {
      $image.setAttribute("style", null);
      $circle.setAttribute("style", null);
      return;
    }
  ...
  const onSubmitEnter = () => {
    setPreviewSubmitEnter(true);
  };
  const onSubmitBlur = () => {
    setPreviewSubmitEnter(false);
  };
    ...
    <Section>
      ...
      <Wrapper>
        ...
        <CircleWrapper
          ...
          isFullscreen={previewSubmitHovered}
          className={`js-preview-image-wrapper`}
        >
          <Circle className={"js-circle"} />
        </CircleWrapper>
        <ImageProteinWrapper
          isHidden={destinationIndex || previewSubmitHovered}
          className={`js-preview-image-wrapper`}
        >
          <ImageProtein
